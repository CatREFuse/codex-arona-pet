...
swift build
"$ROOT_DIR/script/validate_pet_assets.py" --character plana --character arona --character kotonoha-neo
...
case "$MODE" in
  ...
  --telemetry|telemetry)
    ...
    ;;
  --verify|verify)
    OPEN_PLANA_ROOT="$ROOT_DIR" "$APP_BINARY" --verify-animations plana arona kotonoha-neo
    open_app
    sleep 2
